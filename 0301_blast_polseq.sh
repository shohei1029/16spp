...
OUTPUT_FILE=${BLAST_OUT_IRIS}/${NAME_CORE}.txt

mkdir -p $BLAST_OUT_IRIS #for iris
mkdir -p ../analysis/blast

blastp -query ${INPUT_FILE} -db ${NAME_CORE}.fasta -num_threads 64 -outfmt 7 -evalue 1e-5 -max_target_seqs 99999999 -out ${OUTPUT_FILE}

cp ${OUTPUT_FILE} ../analysis/blast


#end
jobdone blastp ${NAME_CORE}
