...
TARGET_NAME=tat
PROT_LENGTH=86
NAME_CORE=HIV-1-all_${TARGET_NAME}-aa
...
python getseq_ingbk_withbginfo_tofasta.py -t ${TARGET_NAME} -m o -o HIV-1| python filter_fasta_by_seq-length.py -n True -r ${PROT_LENGTH} > $FASTA_FILE
...
cd $BLASTDB
makeblastdb -in ${FASTA_FILE_NAME} -dbtype prot -hash_index -parse_seqids -max_file_sz 2GB
